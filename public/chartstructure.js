...
let chartMode = localStorage.getItem("chartStructureMode") || "line";
const chartState = {
...
function renderObOverlays(candles, visibleWindow, geometry, yFor) {
  ...
  if (countNode) countNode.textContent = "Structure overlays: parked";
  return "";
}
...
function renderChartStructure() {
  ...
  if (!candles.length) {
    ...
    container.textContent = "No materialized structure candles recorded yet.";
    if (title) title.textContent = "Waiting for structure candles";
    if (summary) summary.textContent = "Send 1m candle_details webhooks to build materialized MTF candles.";
    if (overlayCount) overlayCount.textContent = "Structure overlays: parked";
    return;
  }
  ...
  if (summary) {
    const closedCount = candles.filter((candle) => candle.status === "closed").length;
    const formingCount = candles.filter((candle) => candle.status === "forming").length;
    summary.textContent = `${visible.length} of ${candles.length} candles · ${leftIndex + 1}-${rightIndex + 1} · materialized from 1m · closed ${closedCount} · forming ${formingCount} · latest ${formatTimestamp(latest.barTime)} · O ${formatPrice(latest.open)} H ${formatPrice(latest.high)} L ${formatPrice(latest.low)} C ${formatPrice(latest.close)}`;
  }
...
async function loadCandles() {
  ...
  const res = await fetch(`/api/structure-candles?${params.toString()}`);
...
async function loadAvailableSymbols() {
  const res = await fetch("/api/structure-candle-symbols");
  ...
  const storedSymbol = localStorage.getItem("chartStructureSymbol") || "";
...
function setupChartStructureControls() {
  ...
  if (modeButton) {
    ...
    modeButton.addEventListener("click", () => {
      ...
      localStorage.setItem("chartStructureMode", chartMode);
      modeButton.textContent = chartMode === "line" ? "Line Mode" : "Candle Mode";
      renderChartStructure();
    });
  }
  ...
  if (symbolSelect) {
    symbolSelect.addEventListener("change", () => {
      localStorage.setItem("chartStructureSymbol", symbolSelect.value);
  ...
  if (timeframeSelect) {
    timeframeSelect.value = localStorage.getItem("chartStructureTimeframe") || "15m";
    timeframeSelect.addEventListener("change", () => {
      localStorage.setItem("chartStructureTimeframe", timeframeSelect.value);
      chartViewport.rightIndex = null;
      chartViewport.stickToLatest = true;
      chartViewport.priceManual = false;
      loadChartData();
...
setupChartStructureControls();
loadAvailableSymbols().then(loadChartData).catch((error) => {
  console.error("Failed to load structure candles:", error);
  const summary = document.getElementById("chart-summary");
  if (summary) summary.textContent = `Failed to load structure candles: ${error.message}`;
});
